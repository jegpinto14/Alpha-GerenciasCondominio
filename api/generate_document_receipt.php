<?php
/**
 * Generador de PDF para recibos de documentos/servicios administrativos
 * 
 * @description Genera PDFs para recibos de servicios como cartas residenciales
...
/**
 * Función para generar el HTML del recibo de documento
 */
function generateDocumentReceiptHTML($recibo) {
    ...
    <head>
        <meta charset='UTF-8'>
        <title>Recibo de Documento</title>
        <style>
            * {
                margin: 0;
                padding: 0;
    ...
    <body>
        <div class='receipt-container'>
            <div class='header'>
                ...
                <div class='system-title'>Sistema de Gestión Administrativa</div>
                <div class='document-title'>Recibo de Servicio</div>
            </div>
            ...
            <div class='info-section'>
                ...
                <div class='info-grid'>
                    ...
                    <div class='info-row'>
                        ...
                        <span class='info-value'>Servicios Administrativos</span>
                    </div>
                    <div class='info-row'>
            ...
            <div class='footer'>
                <div class='validity-statement'>
                    Este documento es válido como comprobante de servicio administrativo.
                </div>
                <div class='system-reference'>
                    Gerencia Express - Sistema de Gestión Administrativa
                </div>
...
/**
 * Función para generar el recibo PDF de documento
 */
function generateDocumentReceipt($detalleId) {
    global $pdo;
    
    try {
        // Obtener datos completos del recibo de documento
        $sql = "
            SELECT 
                di.detalle_id,
                di.ingreso_id,
        ...
        if (!$recibo) {
            throw new Exception("Recibo de documento no encontrado");
        }
        ...
        $pdf->SetTitle('Recibo de Documento');
        $pdf->SetSubject('Recibo de Servicio Administrativo');
        ...
        if ($logoPath && file_exists($logoPath)) {
            ...
            $pdf->Cell(0, 4, 'Sistema de Gestion de Documentos', 0, 1, 'C', 0, '', 0);
        }
        ...
        $pdf->SetY(34);
        $pdf->Cell(0, 8, 'Comprobante de Servicio Administrativo', 0, 1, 'C', 0, '', 0);
        ...
        $pdf->Cell(0, 8, 'INFORMACION DEL SERVICIO', 1, 1, 'C', 1, '', 0);
        ...
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(50, 7, 'Tipo de Servicio:', 1, 0, 'L', 1, '', 0);
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(0, 7, 'Servicio Administrativo', 1, 1, 'L', 0, '', 0);
        
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(50, 7, 'Documento:', 1, 0, 'L', 1, '', 0);
        $pdf->SetFont('helvetica', '', 10);
        ...
        $pdf->Cell(55, 8, 'SERVICIO', 1, 0, 'C', 1, '', 0);
        ...
        $pdf->Cell(0, 5, 'Este documento es valido como comprobante de servicio administrativo.', 0, 1, 'C', 0, '', 0);
        
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetTextColor($azulOscuro[0], $azulOscuro[1], $azulOscuro[2]);
        $pdf->Cell(0, 5, 'Gerencia Express - Sistema de Gestion de Documentos', 0, 1, 'C', 0, '', 0);
        ...
        $fileName = 'Recibo_Documento_' . $detalleId . '_' . date('Ymd_His') . '.pdf';
        
        error_log("Generando PDF de documento con nombre: " . $fileName);
        ...
        error_log("=== FIN generate_document_receipt.php (exitoso) ===");
        exit;
        
    } catch (Exception $e) {
        error_log("Error generando recibo de documento PDF: " . $e->getMessage());
        return false;
    }
}
...
try {
    ...
    $result = generateDocumentReceipt($detalleId);
    ...
    error_log("Error en generate_document_receipt.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
