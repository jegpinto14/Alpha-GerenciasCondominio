<?php
/**
 * Generador de PDF para recibos de artículos
 * 
 * @description Genera PDFs para recibos de compra de artículos (categoría 1)
...
/**
 * Función para generar el HTML del recibo de artículo
 */
function generateArticleReceiptHTML($recibo) {
    ...
    <head>
        <meta charset='UTF-8'>
        <title>Recibo de Artículo</title>
        <style>
            * {
                margin: 0;
                padding: 0;
    ...
    <body>
        <div class='receipt-container'>
            <div class='header'>
                ...
                <div class='system-title'>Sistema de Gestión de Artículos</div>
                <div class='document-title'>Recibo de Compra</div>
            </div>
            
            <div class='info-section'>
                <div class='section-header'>Información de la Compra</div>
                <div class='info-grid'>
                    <div class='info-row'>
                    ...
                    <div class='info-row'>
                        ...
                        <span class='info-value'>Artículos</span>
                    </div>
                    <div class='info-row'>
            ...
            <div class='info-section'>
                <div class='section-header'>Información del Comprador</div>
                <div class='info-grid'>
                    <div class='info-row'>
            ...
            <div class='footer'>
                <div class='validity-statement'>
                    Este documento es válido como comprobante de compra de artículos.
                </div>
                <div class='system-reference'>
                    ARCORUI - Sistema de Gestión de Artículos
                </div>
...
/**
 * Función para generar el recibo PDF de artículo
 */
function generateArticleReceipt($detalleId) {
    global $pdo;
    
    try {
        // Obtener datos completos del recibo de artículo
        $sql = "
            SELECT 
                di.detalle_id,
                di.ingreso_id,
        ...
        if (!$recibo) {
            throw new Exception("Recibo de artículo no encontrado");
        }
        ...
        $pdf->SetTitle('Recibo de Artículo');
        $pdf->SetSubject('Recibo de Compra de Artículo');
        ...
        if (file_exists($logoPath)) {
            ...
            $pdf->Cell(0, 4, 'Sistema de Gestion de Articulos', 0, 1, 'C', 0, '', 0);
        }
        ...
        $pdf->SetY(32);
        $pdf->Cell(0, 8, 'Comprobante de Compra de Articulo', 0, 1, 'C', 0, '', 0);
        ...
        $pdf->Cell(0, 8, 'INFORMACION DE LA COMPRA', 1, 1, 'C', 1, '', 0);
        ...
        $pdf->Cell(50, 7, 'Fecha de Compra:', 1, 0, 'L', 1, '', 0);
        $pdf->SetFont('helvetica', '', 10);
        ...
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(50, 7, 'Tipo de Compra:', 1, 0, 'L', 1, '', 0);
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(0, 7, 'Compra de Articulo', 1, 1, 'L', 0, '', 0);
        
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(50, 7, 'Articulo:', 1, 0, 'L', 1, '', 0);
        $pdf->SetFont('helvetica', '', 10);
        ...
        // TABLA DE DETALLES DEL ARTÍCULO
        $pdf->SetFillColor($azulOscuro[0], $azulOscuro[1], $azulOscuro[2]);
        $pdf->SetTextColor(255, 255, 255);
        $pdf->SetFont('helvetica', 'B', 9);
        
        $pdf->Cell(55, 8, 'ARTICULO', 1, 0, 'C', 1, '', 0);
        ...
        $pdf->Cell(0, 5, 'Este documento es valido como comprobante de compra de articulo.', 0, 1, 'C', 0, '', 0);
        
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->SetTextColor($azulOscuro[0], $azulOscuro[1], $azulOscuro[2]);
        $pdf->Cell(0, 5, 'ARCORUI - Sistema de Gestion de Articulos', 0, 1, 'C', 0, '', 0);
        ...
        $fileName = 'Recibo_Articulo_' . $detalleId . '_' . date('Ymd_His') . '.pdf';
        
        error_log("Generando PDF de artículo con nombre: " . $fileName);
        ...
        error_log("=== FIN generate_article_receipt.php (exitoso) ===");
        exit;
        
    } catch (Exception $e) {
        error_log("Error generando recibo de artículo PDF: " . $e->getMessage());
        return false;
    }
}
...
try {
    ...
    $result = generateArticleReceipt($detalleId);
    ...
    error_log("Error en generate_article_receipt.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
